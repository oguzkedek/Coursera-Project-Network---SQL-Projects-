...
/*
The result set returns all records of departments_dup table ,plus where it match with department managers 
*/
...
SELECT e.emp_no, e.first_name, e.last_name, e.hire_date, d.dept_no, d.from_date
FROM employees e
LEFT JOIN dept_manager d
ON e.emp_no = d.emp_no
...
SELECT e.gender, ROUND(AVG(s.salary),2) AS avg_salary_gender
FROM employees e
JOIN salaries s
ON e.emp_no = s.emp_no
GROUP BY gender
...
/*
Returns the mean of different genders for each emp_no
*/
...
SELECT e.gender, COUNT(d.emp_no) AS count_manager
FROM employees e
JOIN dept_manager d
ON e.emp_no = d.emp_no
GROUP BY e.gender;
...
/*
It will give the same results with the above query .
*/
...
SELECT d.dept_name, ROUND(AVG(s.salary),3) AS avg_salary_dept
FROM departments d
JOIN dept_emp de
...
ON de.emp_no = s.emp_no
GROUP BY d.dept_name
HAVING ROUND(AVG(s.salary),3) > 60000
ORDER BY 2 DESC;