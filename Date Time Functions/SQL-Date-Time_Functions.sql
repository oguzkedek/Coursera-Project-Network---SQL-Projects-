...
SELECT first_name
	,last_name
	,birth_date
	,EXTRACT(YEAR FROM AGE(hire_date, birth_date)) AS hire_age
FROM employees;

-- 3.5: Retrieve a list of how long a manager worked at the company

SELECT emp_no
	,AGE(to_date, from_date) AS Age
FROM dept_manager;
...
/*
There are errors in some data. For example, for a manager whose emp_number is 110039, the age is 8007 years and 3 mons.
*/
...
SELECT order_line, order_date, ship_date, AGE(ship_date, order_date) AS time_
FROM sales
ORDER BY time_ DESC;
...
SELECT first_name
	...
	,birth_date
	,hire_date
	,(EXTRACT(YEAR FROM hire_date) - EXTRACT(YEAR FROM birth_date)) AS emp_age
FROM employees;
...
-- It returns the number of customers aged 25 or less, from each age group 
...
FROM sales
WHERE sub_category = 'chair'
	AND order_id LIKE '%2015';
...
SELECT
	 TO_CHAR(ship_date, 'Month') AS sales_month
	,SUM(sales) AS monthly_total
FROM sales
WHERE sub_category = 'chair'
	AND order_id LIKE '%2015'
GROUP BY sales_month, EXTRACT(MONTH FROM ship_date)
ORDER BY EXTRACT(MONTH FROM ship_date);


-- 7.7: Add a currency (dollars) to the sum of monthly sales

SELECT EXTRACT(MONTH FROM ship_date) AS sales_month
	,TO_CHAR(ship_date, 'Month') AS full_sales_month
	,TO_CHAR(SUM(sales), 'L99999.99') AS monthly_total
FROM sales
