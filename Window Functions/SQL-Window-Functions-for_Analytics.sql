...
SELECT * FROM (
SELECT department,first_name,email, salary,
RANK() OVER(PARTITION BY department ORDER BY salary DESC) rank_n
FROM employees) ranked_salary
WHERE rank_n=5
...
SELECT rank_of_salary, ROUND(AVG(salary),3) AS avg_salary
FROM salary_ranks
...
SUM(salary) OVER(ORDER BY hire_date) AS tot_salary
FROM employees;

-- 5.4: Total Salary for each department

SELECT department, 
SUM(salary) OVER(PARTITION BY department) AS tot_sal_dep
FROM employees;
...
--It'll return the sum of four rows. 
